...
impl<D: TableDelegate + 'static> TableState<D> {
    ...
    pub fn edit_selection(&mut self, cx: &mut Context<Self>) {
    ...
    pub(crate) fn update_bounds(&mut self, bounds: Bounds<Pixels>, cx: &mut Context<Self>) {
        if self.bounds == bounds {
            return;
        };
        self.bounds = bounds;

        self.reset_column_widths(self.bounds, cx);
        cx.notify();
    }

    fn reset_column_widths(&mut self, bounds: Bounds<Pixels>, cx: &mut Context<Self>) {
        ...
        let min_widths: Vec<Pixels> =
            (0..col_count).map(|col_ix| self.delegate.column(col_ix, cx).min_width()).collect();

        let total_min: Pixels = min_widths.iter().copied().sum();
        if total_min >= bounds.size.width {
            self.column_widths = min_widths;
            return;
        }

        self.column_widths.clear();
        self.column_widths.extend(min_widths[..col_count - 1].iter().copied());

        let taken: Pixels = self.column_widths.iter().copied().sum();
        self.column_widths.push((bounds.size.width - taken).max(px(0.0)));
    }
...
impl<D: TableDelegate> RowRegistry<D> {
    ...
    pub fn expand_path_to(&mut self, row_id: &D::RowId) {
        let Some(mut ix) = self.indices.get(row_id).copied() else {
            return;
        };
    ...
    pub fn expand_parents(&mut self, row_id: &D::RowId) {
        ...
        let Some(parent_ix) = node.parent.as_ref() else {
            return;
        };
