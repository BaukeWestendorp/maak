...
use gpui::{
    ...
    MouseDownEvent, MouseMoveEvent, Pixels, Window, div, uniform_list,
};
...
use crate::{ActiveTheme, ElementExt, Icon, IconSize, IconVariant, h_flex};
...
impl<D: TableDelegate + 'static> RenderOnce for Table<D> {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let table = div()
            .id(ElementId::named_usize("table", self.state.entity_id().as_u64() as usize))
            .size_full()
            .flex()
            .flex_col()
            ...
            .on_prepaint({
                ...
                move |new_bounds, _, cx| {
                    // FIXME: This still is kind of broken and does weird stuff when resizing (not responding immediately).
                    state.update(cx, |state, cx| {
                        state.update_bounds(new_bounds, cx);
                    })
                }
            })
...
impl<D: TableDelegate + 'static> TableState<D> {
    ...
    pub fn render_body(&self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        .flex()
        .flex_col()
        .size_full() // FIXME: SCROLLING FOR ROWS IS NOT WORKING, AND ALSO IMPLEMENT HORIZONTAL SCROLLING
